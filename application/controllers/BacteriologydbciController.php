...
class BacteriologydbciController extends Zend_Controller_Action
{
        ...
        if ($dataDB != false) {
            foreach ($dataDB as $key => $value) {
                if ($tableName == 'tbl_bac_panels_shipments') {
                    ...
                    $dataDB[$key]->roundStatus = $round['status'];
    ...
    public function updateShipmentRelatedTables($where, $update)
    {
        ...
        $whereShipmentData = $this->dbConnection->selectFromTable('tbl_bac_panels_shipments',
            ['participantId' => $where['participantId'], 'shipmentId' => $shipmentId]);
    ...
    public function savesamplesToUsers($jsPostData)
    {
        try {

...
class Application_Service_Participants
{
    ...
    {
        $participantDb = new Application_Model_DbTable_Participants();
        return $participantDb->addParticipants($params);
    }
    ...
    public function getUserResetCodeDetails($resetCode)
    {

        $db = Zend_Db_Table_Abstract::getDefaultAdapter();
        $sql = $db->select()
            ->from(array('p' => 'data_manager'),array('dm_id','primary_email','resetCode','first_name','force_password_reset'))
    ...
    public function updatefromcodePassword($newpassword,$id) {

        $db = Zend_Db_Table_Abstract::getDefaultAdapter();
        $noOfRows = $db->update('data_manager',array('password' => md5($newpassword), 'force_password_reset' => 0), "dm_id = '" . $id . "' ");

...
* GlobalConfig
*  
* Low level class for manipulating the data in the table global_config
*
...
class GlobalConfig {
	...
	*/
     ...
     * based on the value of $name,$value passed to the function
     *
     * @param $name,$value
	...
	public function getName($name,$value) {
		...
		$name_ = $this->query_from_global_config ( $columns, $records );
     ...
     * based on the value of $name passed to the function
     *
     * @param $name
	...
	public function getValue($name) {
		$columns = array ('name');
		$records = array ($name);
		$value_ = $this->query_from_global_config ( $columns, $records );
	...
	*/
	public function insert_prepared_records($name,$value,$redundancy_check= false, $printSQL = false) {
		$columns = array('name','value');
		$records = array($name,$value);
		return $this->insert_records_to_global_config ( $columns, $records,$redundancy_check, $printSQL );
	}

	/**
    * Delete data from the table[global_config] in the order below
    * array ('name','value')
    * is mapped into
    * array ($name,$value)
    * @return int number of deleted rows
    */
    public function delete_prepared_records($name,$value, $printSQL = false) {
    	$columns = array('name','value');
    	$records = array($name,$value);
    	return $this->delete_record_from_global_config( $columns, $records, $printSQL );
    }
	...
	* @return string table name -> 'global_config' 
	*/
	...
	*/
	...
	/**
     * Used  to calculate the number of times a record exists in the table global_config
     * It returns the number of times a record exists exists in the table global_config
     * @param array $columns
     * @param array $records
     * @param bool $printSQL
     * @return mixed
	...
	/**
     * Inserts data into the table global_config
     * if redundancy check is true, it inserts if the record if it never existed else.
     * if the record exists, it returns the number of times the record exists on the relation
     *
     * @param array $columns
	...
	/**
	 * Deletes all the records that meets the passed criteria from the table global_config
	 * @param array $columns
	 * @param array $records
	 * @param bool $printSQL
	 * @return number of deleted rows
	 */
	public function delete_record_from_global_config(Array $columns, Array $records, $printSQL = false) {
		return $this->delete_record ( $this->get_table (), $columns, $records, $printSQL );
	}
	
	/**
	 * Updates all the records that meets the passed criteria from the table global_config
	 * 
	 * @param array $columns
	 * @param array $records
	 ...
	 */
	public function update_record_in_global_config(Array $columns, Array $records, Array $where_columns, Array $where_records, $printSQL = false) {
		return $this->update_record ( $this->get_table (), $columns, $records, $where_columns, $where_records, $printSQL );
	}
	
	/**
	 * Gets an Associative array of the records in the table 'global_config' that meets the passed criteria
	 *
	 * @param $distinct
	 * @param array $columns
	 * @param array $records
	 ...
	 */
	public function fetch_assoc_in_global_config($distinct, Array $columns, Array $records, $extraSQL="", $printSQL = false) {
	 ...
	 * @return array|mixed associative
	 */
	public function query_from_global_config(Array $columns, Array $records,$extraSQL="",  $printSQL = false) {
		return $this->query ( $this->get_table (), $columns, $records,$extraSQL,$printSQL );
	}
	
	/**
	 * Gets an Associative array of the records in the table global_config that meets the passed distinct criteria
	 * 
	 * @param array $columns
	 * @param array $records
	 * @param string $extraSQL
	 ...
	 * @return array|mixed associative
	 */
	public function query_distinct_from_global_config(Array $columns, Array $records,$extraSQL="",  $printSQL = false) {
		return $this->query_distinct ( $this->get_table (), $columns, $records,$extraSQL,$printSQL );
	}
	
	/**
	 * Performs a search in the table global_config that meets the passed criteria
	 * 
	 * @param array $columns
	 * @param array $records
	 * @param string $extraSQL
	 ...
	 * @return array|mixed associative
	 */
	public function search_in_global_config(Array $columns, Array $records,$extraSQL="", $printSQL = false) {
   ...
   * @throws NullabilityException
	...
	/**
     * Gets an Associative array of the records in the table global_config that meets the distinct passed criteria
     * @param $table
     * @param array $columns
     * @param array $records
     * @param string $extraSQL
	...
	private function query_distinct($table, Array $columns, Array $records,$extraSQL="",$printSQL = false) {
	 ...
	 /**
     * Performs a search and returns an associative array of the records in the table global_config  that meets the passed criteria
     * 
     * @param $table
     * @param array $columns
     * @param array $records

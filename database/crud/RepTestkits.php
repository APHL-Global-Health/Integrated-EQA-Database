...
* RepTestkits
*  
* Low level class for manipulating the data in the table rep_testkits
*
...
class RepTestkits {
	...
	*/
     ...
     * based on the value of $ID,$TestKitName,$ProgramID,$ProviderID,$CreatedBy,$CreatedDate passed to the function
     *
     * @param $ID,$TestKitName,$ProgramID,$ProviderID,$CreatedBy,$CreatedDate
     * @return object (ID)| null
     */
	public function getID($ID,$TestKitName,$ProgramID,$ProviderID,$CreatedBy,$CreatedDate) {
		$columns = array ('ID','TestKitName','ProgramID','ProviderID','CreatedBy','CreatedDate');
		$records = array ($ID,$TestKitName,$ProgramID,$ProviderID,$CreatedBy,$CreatedDate);
		$ID_ = $this->query_from_rep_testkits ( $columns, $records );
		return sizeof($ID_)>0 ? $ID_ [0] ['ID'] : null;
	}
	
	
	/**
     * Performs a database query and returns the value of TestKitName 
     * based on the value of $ID passed to the function
     *
     * @param $ID
     * @return object (TestKitName)| null
     */
	public function getTestKitName($ID) {
		$columns = array ('ID');
		$records = array ($ID);
		$TestKitName_ = $this->query_from_rep_testkits ( $columns, $records );
		return sizeof($TestKitName_)>0 ? $TestKitName_ [0] ['TestKitName'] : null;
	}
	...
	public function getProgramID($ID) {
		$columns = array ('ID');
		$records = array ($ID);
		$ProgramID_ = $this->query_from_rep_testkits ( $columns, $records );
	...
	public function getProviderID($ID) {
		$columns = array ('ID');
		$records = array ($ID);
		$ProviderID_ = $this->query_from_rep_testkits ( $columns, $records );
	...
	public function getCreatedBy($ID) {
		$columns = array ('ID');
		$records = array ($ID);
		$CreatedBy_ = $this->query_from_rep_testkits ( $columns, $records );
	...
	public function getCreatedDate($ID) {
		$columns = array ('ID');
		$records = array ($ID);
		$CreatedDate_ = $this->query_from_rep_testkits ( $columns, $records );
	   ...
	   /**
	* Inserts data into the table[rep_testkits] in the order below
	* array ('ID','TestKitName','ProgramID','ProviderID','CreatedBy','CreatedDate')
	* is mapped into
	* array ($ID,$TestKitName,$ProgramID,$ProviderID,$CreatedBy,$CreatedDate)
	...
	*/
	public function insert_prepared_records($ID,$TestKitName,$ProgramID,$ProviderID,$CreatedBy,$CreatedDate,$redundancy_check= false, $printSQL = false) {
		$columns = array('ID','TestKitName','ProgramID','ProviderID','CreatedBy','CreatedDate');
		$records = array($ID,$TestKitName,$ProgramID,$ProviderID,$CreatedBy,$CreatedDate);
		return $this->insert_records_to_rep_testkits ( $columns, $records,$redundancy_check, $printSQL );
	}

	/**
    * Delete data from the table[rep_testkits] in the order below
    * array ('ID','TestKitName','ProgramID','ProviderID','CreatedBy','CreatedDate')
    * is mapped into
    * array ($ID,$TestKitName,$ProgramID,$ProviderID,$CreatedBy,$CreatedDate)
    * @return int number of deleted rows
    */
    public function delete_prepared_records($ID,$TestKitName,$ProgramID,$ProviderID,$CreatedBy,$CreatedDate, $printSQL = false) {
    	$columns = array('ID','TestKitName','ProgramID','ProviderID','CreatedBy','CreatedDate');
    	$records = array($ID,$TestKitName,$ProgramID,$ProviderID,$CreatedBy,$CreatedDate);
    	return $this->delete_record_from_rep_testkits( $columns, $records, $printSQL );
    }
	...
	* @return string table name -> 'rep_testkits' 
	*/
	...
	*/
	...
	/**
     * Used  to calculate the number of times a record exists in the table rep_testkits
     * It returns the number of times a record exists exists in the table rep_testkits
     * @param array $columns
     * @param array $records
     * @param bool $printSQL
     * @return mixed
	...
	/**
     * Inserts data into the table rep_testkits
     * if redundancy check is true, it inserts if the record if it never existed else.
     * if the record exists, it returns the number of times the record exists on the relation
     *
     * @param array $columns
     ...
     * @return mixed
     */
	public function insert_records_to_rep_testkits(Array $columns, Array $records,$redundancy_check = false, $printSQL = false) {
	...
	/**
	 * Deletes all the records that meets the passed criteria from the table rep_testkits
	 * @param array $columns
	 * @param array $records
	 * @param bool $printSQL
	 * @return number of deleted rows
	 */
	public function delete_record_from_rep_testkits(Array $columns, Array $records, $printSQL = false) {
		return $this->delete_record ( $this->get_table (), $columns, $records, $printSQL );
	}
	
	/**
	 * Updates all the records that meets the passed criteria from the table rep_testkits
	 * 
	 * @param array $columns
	 * @param array $records
	 ...
	 */
	public function update_record_in_rep_testkits(Array $columns, Array $records, Array $where_columns, Array $where_records, $printSQL = false) {
		return $this->update_record ( $this->get_table (), $columns, $records, $where_columns, $where_records, $printSQL );
	}
	
	/**
	 * Gets an Associative array of the records in the table 'rep_testkits' that meets the passed criteria
	 *
	 * @param $distinct
	 * @param array $columns
	 * @param array $records
	 ...
	 */
	public function fetch_assoc_in_rep_testkits($distinct, Array $columns, Array $records, $extraSQL="", $printSQL = false) {
	 ...
	 * @return array|mixed associative
	 */
	public function query_from_rep_testkits(Array $columns, Array $records,$extraSQL="",  $printSQL = false) {
		return $this->query ( $this->get_table (), $columns, $records,$extraSQL,$printSQL );
	}
	
	/**
	 * Gets an Associative array of the records in the table rep_testkits that meets the passed distinct criteria
	 * 
	 * @param array $columns
	 * @param array $records
	 * @param string $extraSQL
	 ...
	 * @return array|mixed associative
	 */
	public function query_distinct_from_rep_testkits(Array $columns, Array $records,$extraSQL="",  $printSQL = false) {
		return $this->query_distinct ( $this->get_table (), $columns, $records,$extraSQL,$printSQL );
	}
	
	/**
	 * Performs a search in the table rep_testkits that meets the passed criteria
	 * 
	 * @param array $columns
	 * @param array $records
	 * @param string $extraSQL
	 ...
	 * @return array|mixed associative
	 */
	public function search_in_rep_testkits(Array $columns, Array $records,$extraSQL="", $printSQL = false) {
   ...
   * @throws NullabilityException
	...
	/**
     * Gets an Associative array of the records in the table rep_testkits that meets the distinct passed criteria
     * @param $table
     * @param array $columns
     * @param array $records
     * @param string $extraSQL
	...
	private function query_distinct($table, Array $columns, Array $records,$extraSQL="",$printSQL = false) {
	 ...
	 /**
     * Performs a search and returns an associative array of the records in the table rep_testkits  that meets the passed criteria
     * 
     * @param $table
     * @param array $columns
     * @param array $records

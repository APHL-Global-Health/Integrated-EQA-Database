...
* RSiteType
*  
* Low level class for manipulating the data in the table r_site_type
*
...
class RSiteType {
	...
	/**
	* private class variable $_rStid
	*/
	private $_rStid;
	
	/**
	* returns the value of $rStid
	*
	* @return object(int|string) rStid
	*/
	public function _getRStid() {
		return $this->_rStid;
	}
	
	/**
	* sets the value of $_rStid
	*
	* @param rStid
	*/
	public function _setRStid($rStid) {
		$this->_rStid = $rStid;
	}
	/**
	* sets the value of $_rStid
	...
	*/
	public function setRStid($rStid) {
		$this->_setRStid($rStid);
		return $this;
	}
	...
	*/
	...
	/**
     * Performs a database query and returns the value of r_stid 
     * based on the value of $r_stid,$site_type passed to the function
     *
     * @param $r_stid,$site_type
     * @return object (r_stid)| null
     */
	public function getRStid($r_stid,$site_type) {
		$columns = array ('r_stid','site_type');
		$records = array ($r_stid,$site_type);
		$r_stid_ = $this->query_from_r_site_type ( $columns, $records );
		return sizeof($r_stid_)>0 ? $r_stid_ [0] ['r_stid'] : null;
	}
	
	
	/**
     * Performs a database query and returns the value of site_type 
     * based on the value of $r_stid passed to the function
     *
     * @param $r_stid
     * @return object (site_type)| null
     */
	public function getSiteType($r_stid) {
		$columns = array ('r_stid');
		$records = array ($r_stid);
		$site_type_ = $this->query_from_r_site_type ( $columns, $records );
	...
	*/
	public function insert_prepared_records($r_stid,$site_type,$redundancy_check= false, $printSQL = false) {
		$columns = array('r_stid','site_type');
		$records = array($r_stid,$site_type);
		return $this->insert_records_to_r_site_type ( $columns, $records,$redundancy_check, $printSQL );
	}

	/**
    * Delete data from the table[r_site_type] in the order below
    * array ('r_stid','site_type')
    * is mapped into
    * array ($r_stid,$site_type)
    * @return int number of deleted rows
    */
    public function delete_prepared_records($r_stid,$site_type, $printSQL = false) {
    	$columns = array('r_stid','site_type');
    	$records = array($r_stid,$site_type);
    	return $this->delete_record_from_r_site_type( $columns, $records, $printSQL );
    }
	...
	* @return string table name -> 'r_site_type' 
	*/
	...
	*/
	...
	/**
     * Used  to calculate the number of times a record exists in the table r_site_type
     * It returns the number of times a record exists exists in the table r_site_type
     * @param array $columns
     * @param array $records
     * @param bool $printSQL
     * @return mixed
	...
	/**
     * Inserts data into the table r_site_type
     * if redundancy check is true, it inserts if the record if it never existed else.
     * if the record exists, it returns the number of times the record exists on the relation
     *
     * @param array $columns
	...
	/**
	 * Deletes all the records that meets the passed criteria from the table r_site_type
	 * @param array $columns
	 * @param array $records
	 * @param bool $printSQL
	 * @return number of deleted rows
	 */
	public function delete_record_from_r_site_type(Array $columns, Array $records, $printSQL = false) {
		return $this->delete_record ( $this->get_table (), $columns, $records, $printSQL );
	}
	
	/**
	 * Updates all the records that meets the passed criteria from the table r_site_type
	 * 
	 * @param array $columns
	 * @param array $records
	 ...
	 */
	public function update_record_in_r_site_type(Array $columns, Array $records, Array $where_columns, Array $where_records, $printSQL = false) {
		return $this->update_record ( $this->get_table (), $columns, $records, $where_columns, $where_records, $printSQL );
	}
	
	/**
	 * Gets an Associative array of the records in the table 'r_site_type' that meets the passed criteria
	 *
	 * @param $distinct
	 * @param array $columns
	 * @param array $records
	 ...
	 */
	public function fetch_assoc_in_r_site_type($distinct, Array $columns, Array $records, $extraSQL="", $printSQL = false) {
	 ...
	 * @return array|mixed associative
	 */
	public function query_from_r_site_type(Array $columns, Array $records,$extraSQL="",  $printSQL = false) {
		return $this->query ( $this->get_table (), $columns, $records,$extraSQL,$printSQL );
	}
	
	/**
	 * Gets an Associative array of the records in the table r_site_type that meets the passed distinct criteria
	 * 
	 * @param array $columns
	 * @param array $records
	 * @param string $extraSQL
	 ...
	 * @return array|mixed associative
	 */
	public function query_distinct_from_r_site_type(Array $columns, Array $records,$extraSQL="",  $printSQL = false) {
		return $this->query_distinct ( $this->get_table (), $columns, $records,$extraSQL,$printSQL );
	}
	
	/**
	 * Performs a search in the table r_site_type that meets the passed criteria
	 * 
	 * @param array $columns
	 * @param array $records
	 * @param string $extraSQL
	 ...
	 * @return array|mixed associative
	 */
	public function search_in_r_site_type(Array $columns, Array $records,$extraSQL="", $printSQL = false) {
   ...
   * @throws NullabilityException
	...
	/**
     * Gets an Associative array of the records in the table r_site_type that meets the distinct passed criteria
     * @param $table
     * @param array $columns
     * @param array $records
     * @param string $extraSQL
	...
	private function query_distinct($table, Array $columns, Array $records,$extraSQL="",$printSQL = false) {
	 ...
	 /**
     * Performs a search and returns an associative array of the records in the table r_site_type  that meets the passed criteria
     * 
     * @param $table
     * @param array $columns
     * @param array $records

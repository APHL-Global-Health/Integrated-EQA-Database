...
* ReferenceDtsEia
*  
* Low level class for manipulating the data in the table reference_dts_eia
*
...
class ReferenceDtsEia {
	...
	*/
	...
	public function getId($id,$shipment_id,$sample_id,$eia,$lot,$exp_date,$od,$cutoff) {
		...
		$id_ = $this->query_from_reference_dts_eia ( $columns, $records );
	...
	public function getShipmentId($id) {
		$columns = array ('id');
		$records = array ($id);
		$shipment_id_ = $this->query_from_reference_dts_eia ( $columns, $records );
	...
	public function getSampleId($id) {
		$columns = array ('id');
		$records = array ($id);
		$sample_id_ = $this->query_from_reference_dts_eia ( $columns, $records );
	...
	public function getEia($id) {
		$columns = array ('id');
		$records = array ($id);
		$eia_ = $this->query_from_reference_dts_eia ( $columns, $records );
	...
	public function getLot($id) {
		$columns = array ('id');
		$records = array ($id);
		$lot_ = $this->query_from_reference_dts_eia ( $columns, $records );
	...
	public function getExpDate($id) {
		$columns = array ('id');
		$records = array ($id);
		$exp_date_ = $this->query_from_reference_dts_eia ( $columns, $records );
	...
	public function getOd($id) {
		$columns = array ('id');
		$records = array ($id);
		$od_ = $this->query_from_reference_dts_eia ( $columns, $records );
	...
	public function getCutoff($id) {
		$columns = array ('id');
		$records = array ($id);
		$cutoff_ = $this->query_from_reference_dts_eia ( $columns, $records );
	...
	*/
	public function insert_prepared_records($id,$shipment_id,$sample_id,$eia,$lot,$exp_date,$od,$cutoff,$redundancy_check= false, $printSQL = false) {
		$columns = array('id','shipment_id','sample_id','eia','lot','exp_date','od','cutoff');
		$records = array($id,$shipment_id,$sample_id,$eia,$lot,$exp_date,$od,$cutoff);
		return $this->insert_records_to_reference_dts_eia ( $columns, $records,$redundancy_check, $printSQL );
	}

	/**
    * Delete data from the table[reference_dts_eia] in the order below
    * array ('id','shipment_id','sample_id','eia','lot','exp_date','od','cutoff')
    * is mapped into
    * array ($id,$shipment_id,$sample_id,$eia,$lot,$exp_date,$od,$cutoff)
    ...
    */
    public function delete_prepared_records($id,$shipment_id,$sample_id,$eia,$lot,$exp_date,$od,$cutoff, $printSQL = false) {
    	$columns = array('id','shipment_id','sample_id','eia','lot','exp_date','od','cutoff');
    	$records = array($id,$shipment_id,$sample_id,$eia,$lot,$exp_date,$od,$cutoff);
    	return $this->delete_record_from_reference_dts_eia( $columns, $records, $printSQL );
    }
	...
	* @return string table name -> 'reference_dts_eia' 
	*/
	public static function get_table() {
		return 'reference_dts_eia';
	}
	...
	*/
	...
	/**
     * Used  to calculate the number of times a record exists in the table reference_dts_eia
     * It returns the number of times a record exists exists in the table reference_dts_eia
     * @param array $columns
     * @param array $records
     * @param bool $printSQL
     * @return mixed
	...
	/**
     * Inserts data into the table reference_dts_eia
     * if redundancy check is true, it inserts if the record if it never existed else.
     * if the record exists, it returns the number of times the record exists on the relation
     *
     * @param array $columns
	...
	/**
	 * Deletes all the records that meets the passed criteria from the table reference_dts_eia
	 * @param array $columns
	 * @param array $records
	 * @param bool $printSQL
	 * @return number of deleted rows
	 */
	public function delete_record_from_reference_dts_eia(Array $columns, Array $records, $printSQL = false) {
		return $this->delete_record ( $this->get_table (), $columns, $records, $printSQL );
	}
	
	/**
	 * Updates all the records that meets the passed criteria from the table reference_dts_eia
	 * 
	 * @param array $columns
	 * @param array $records
	 ...
	 */
	public function update_record_in_reference_dts_eia(Array $columns, Array $records, Array $where_columns, Array $where_records, $printSQL = false) {
		return $this->update_record ( $this->get_table (), $columns, $records, $where_columns, $where_records, $printSQL );
	}
	
	/**
	 * Gets an Associative array of the records in the table 'reference_dts_eia' that meets the passed criteria
	 *
	 * @param $distinct
	 * @param array $columns
	 * @param array $records
	 ...
	 */
	public function fetch_assoc_in_reference_dts_eia($distinct, Array $columns, Array $records, $extraSQL="", $printSQL = false) {
	 ...
	 * @return array|mixed associative
	 */
	public function query_from_reference_dts_eia(Array $columns, Array $records,$extraSQL="",  $printSQL = false) {
		return $this->query ( $this->get_table (), $columns, $records,$extraSQL,$printSQL );
	}
	
	/**
	 * Gets an Associative array of the records in the table reference_dts_eia that meets the passed distinct criteria
	 * 
	 * @param array $columns
	 * @param array $records
	 * @param string $extraSQL
	 ...
	 * @return array|mixed associative
	 */
	public function query_distinct_from_reference_dts_eia(Array $columns, Array $records,$extraSQL="",  $printSQL = false) {
		return $this->query_distinct ( $this->get_table (), $columns, $records,$extraSQL,$printSQL );
	}
	
	/**
	 * Performs a search in the table reference_dts_eia that meets the passed criteria
	 * 
	 * @param array $columns
	 * @param array $records
	 * @param string $extraSQL
	 ...
	 * @return array|mixed associative
	 */
	public function search_in_reference_dts_eia(Array $columns, Array $records,$extraSQL="", $printSQL = false) {
   ...
   * @throws NullabilityException
	...
	/**
     * Gets an Associative array of the records in the table reference_dts_eia that meets the distinct passed criteria
     * @param $table
     * @param array $columns
     * @param array $records
     * @param string $extraSQL
	...
	private function query_distinct($table, Array $columns, Array $records,$extraSQL="",$printSQL = false) {
	 ...
	 /**
     * Performs a search and returns an associative array of the records in the table reference_dts_eia  that meets the passed criteria
     * 
     * @param $table
     * @param array $columns
     * @param array $records

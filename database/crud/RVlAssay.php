...
* RVlAssay
*  
* Low level class for manipulating the data in the table r_vl_assay
*
...
class RVlAssay {
	...
	*/
	...
	/**
	* private class variable $_shortName
	*/
	private $_shortName;
	...
	*
	* @return object(int|string) shortName
	*/
	...
	/**
	* sets the value of $_shortName
	*
	* @param shortName
	*/
	public function _setShortName($shortName) {
	...
	*/
     ...
     * based on the value of $id,$name,$short_name passed to the function
     *
     * @param $id,$name,$short_name
     * @return object (id)| null
     */
	public function getId($id,$name,$short_name) {
		$columns = array ('id','name','short_name');
		$records = array ($id,$name,$short_name);
		$id_ = $this->query_from_r_vl_assay ( $columns, $records );
	...
	public function getName($id) {
		$columns = array ('id');
		$records = array ($id);
		$name_ = $this->query_from_r_vl_assay ( $columns, $records );
		return sizeof($name_)>0 ? $name_ [0] ['name'] : null;
	}
	
	
	/**
     * Performs a database query and returns the value of short_name 
     * based on the value of $id passed to the function
     *
     * @param $id
     * @return object (short_name)| null
     */
	public function getShortName($id) {
		$columns = array ('id');
		$records = array ($id);
		$short_name_ = $this->query_from_r_vl_assay ( $columns, $records );
		return sizeof($short_name_)>0 ? $short_name_ [0] ['short_name'] : null;
	}
	...
	*/
	public function insert_prepared_records($id,$name,$short_name,$redundancy_check= false, $printSQL = false) {
		$columns = array('id','name','short_name');
		$records = array($id,$name,$short_name);
		return $this->insert_records_to_r_vl_assay ( $columns, $records,$redundancy_check, $printSQL );
	}

	/**
    * Delete data from the table[r_vl_assay] in the order below
    * array ('id','name','short_name')
    * is mapped into
    * array ($id,$name,$short_name)
    * @return int number of deleted rows
    */
    public function delete_prepared_records($id,$name,$short_name, $printSQL = false) {
    	$columns = array('id','name','short_name');
    	$records = array($id,$name,$short_name);
    	return $this->delete_record_from_r_vl_assay( $columns, $records, $printSQL );
    }
	...
	* @return string table name -> 'r_vl_assay' 
	*/
	...
	*/
	...
	/**
     * Used  to calculate the number of times a record exists in the table r_vl_assay
     * It returns the number of times a record exists exists in the table r_vl_assay
     * @param array $columns
     * @param array $records
     * @param bool $printSQL
     * @return mixed
	...
	/**
     * Inserts data into the table r_vl_assay
     * if redundancy check is true, it inserts if the record if it never existed else.
     * if the record exists, it returns the number of times the record exists on the relation
     *
     * @param array $columns
	...
	/**
	 * Deletes all the records that meets the passed criteria from the table r_vl_assay
	 * @param array $columns
	 * @param array $records
	 * @param bool $printSQL
	 * @return number of deleted rows
	 */
	public function delete_record_from_r_vl_assay(Array $columns, Array $records, $printSQL = false) {
		return $this->delete_record ( $this->get_table (), $columns, $records, $printSQL );
	}
	
	/**
	 * Updates all the records that meets the passed criteria from the table r_vl_assay
	 * 
	 * @param array $columns
	 * @param array $records
	 ...
	 */
	public function update_record_in_r_vl_assay(Array $columns, Array $records, Array $where_columns, Array $where_records, $printSQL = false) {
		return $this->update_record ( $this->get_table (), $columns, $records, $where_columns, $where_records, $printSQL );
	}
	
	/**
	 * Gets an Associative array of the records in the table 'r_vl_assay' that meets the passed criteria
	 *
	 * @param $distinct
	 * @param array $columns
	 * @param array $records
	 ...
	 */
	public function fetch_assoc_in_r_vl_assay($distinct, Array $columns, Array $records, $extraSQL="", $printSQL = false) {
	 ...
	 * @return array|mixed associative
	 */
	public function query_from_r_vl_assay(Array $columns, Array $records,$extraSQL="",  $printSQL = false) {
		return $this->query ( $this->get_table (), $columns, $records,$extraSQL,$printSQL );
	}
	
	/**
	 * Gets an Associative array of the records in the table r_vl_assay that meets the passed distinct criteria
	 * 
	 * @param array $columns
	 * @param array $records
	 * @param string $extraSQL
	 ...
	 * @return array|mixed associative
	 */
	public function query_distinct_from_r_vl_assay(Array $columns, Array $records,$extraSQL="",  $printSQL = false) {
		return $this->query_distinct ( $this->get_table (), $columns, $records,$extraSQL,$printSQL );
	}
	
	/**
	 * Performs a search in the table r_vl_assay that meets the passed criteria
	 * 
	 * @param array $columns
	 * @param array $records
	 * @param string $extraSQL
	 ...
	 * @return array|mixed associative
	 */
	public function search_in_r_vl_assay(Array $columns, Array $records,$extraSQL="", $printSQL = false) {
   ...
   * @throws NullabilityException
	...
	/**
     * Gets an Associative array of the records in the table r_vl_assay that meets the distinct passed criteria
     * @param $table
     * @param array $columns
     * @param array $records
     * @param string $extraSQL
	...
	private function query_distinct($table, Array $columns, Array $records,$extraSQL="",$printSQL = false) {
	 ...
	 /**
     * Performs a search and returns an associative array of the records in the table r_vl_assay  that meets the passed criteria
     * 
     * @param $table
     * @param array $columns
     * @param array $records

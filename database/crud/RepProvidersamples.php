...
* RepProvidersamples
*  
* Low level class for manipulating the data in the table rep_providersamples
*
...
class RepProvidersamples {
	...
	*/
     ...
     * based on the value of $ID,$SampleCode,$ProviderID,$ProgramID,$PeriodID,$LabID,$CreatedBy,$CreatedDate passed to the function
     *
     * @param $ID,$SampleCode,$ProviderID,$ProgramID,$PeriodID,$LabID,$CreatedBy,$CreatedDate
     * @return object (ID)| null
     */
	public function getID($ID,$SampleCode,$ProviderID,$ProgramID,$PeriodID,$LabID,$CreatedBy,$CreatedDate) {
		$columns = array ('ID','SampleCode','ProviderID','ProgramID','PeriodID','LabID','CreatedBy','CreatedDate');
		$records = array ($ID,$SampleCode,$ProviderID,$ProgramID,$PeriodID,$LabID,$CreatedBy,$CreatedDate);
		$ID_ = $this->query_from_rep_providersamples ( $columns, $records );
     ...
     */
	public function getSampleCode($ID) {
		$columns = array ('ID');
		$records = array ($ID);
		$SampleCode_ = $this->query_from_rep_providersamples ( $columns, $records );
	...
	public function getProviderID($ID) {
		$columns = array ('ID');
		$records = array ($ID);
		$ProviderID_ = $this->query_from_rep_providersamples ( $columns, $records );
	...
	public function getProgramID($ID) {
		$columns = array ('ID');
		$records = array ($ID);
		$ProgramID_ = $this->query_from_rep_providersamples ( $columns, $records );
	...
	public function getPeriodID($ID) {
		$columns = array ('ID');
		$records = array ($ID);
		$PeriodID_ = $this->query_from_rep_providersamples ( $columns, $records );
	...
	public function getLabID($ID) {
		$columns = array ('ID');
		$records = array ($ID);
		$LabID_ = $this->query_from_rep_providersamples ( $columns, $records );
	...
	public function getCreatedBy($ID) {
		$columns = array ('ID');
		$records = array ($ID);
		$CreatedBy_ = $this->query_from_rep_providersamples ( $columns, $records );
	...
	public function getCreatedDate($ID) {
		$columns = array ('ID');
		$records = array ($ID);
		$CreatedDate_ = $this->query_from_rep_providersamples ( $columns, $records );
	   ...
	   /**
	* Inserts data into the table[rep_providersamples] in the order below
	* array ('ID','SampleCode','ProviderID','ProgramID','PeriodID','LabID','CreatedBy','CreatedDate')
	* is mapped into
	* array ($ID,$SampleCode,$ProviderID,$ProgramID,$PeriodID,$LabID,$CreatedBy,$CreatedDate)
	...
	*/
	public function insert_prepared_records($ID,$SampleCode,$ProviderID,$ProgramID,$PeriodID,$LabID,$CreatedBy,$CreatedDate,$redundancy_check= false, $printSQL = false) {
		$columns = array('ID','SampleCode','ProviderID','ProgramID','PeriodID','LabID','CreatedBy','CreatedDate');
		$records = array($ID,$SampleCode,$ProviderID,$ProgramID,$PeriodID,$LabID,$CreatedBy,$CreatedDate);
		return $this->insert_records_to_rep_providersamples ( $columns, $records,$redundancy_check, $printSQL );
	}

	/**
    * Delete data from the table[rep_providersamples] in the order below
    * array ('ID','SampleCode','ProviderID','ProgramID','PeriodID','LabID','CreatedBy','CreatedDate')
    * is mapped into
    * array ($ID,$SampleCode,$ProviderID,$ProgramID,$PeriodID,$LabID,$CreatedBy,$CreatedDate)
    * @return int number of deleted rows
    */
    public function delete_prepared_records($ID,$SampleCode,$ProviderID,$ProgramID,$PeriodID,$LabID,$CreatedBy,$CreatedDate, $printSQL = false) {
    	$columns = array('ID','SampleCode','ProviderID','ProgramID','PeriodID','LabID','CreatedBy','CreatedDate');
    	$records = array($ID,$SampleCode,$ProviderID,$ProgramID,$PeriodID,$LabID,$CreatedBy,$CreatedDate);
    	return $this->delete_record_from_rep_providersamples( $columns, $records, $printSQL );
    }
	...
	* @return string table name -> 'rep_providersamples' 
	*/
	public static function get_table() {
		return 'rep_providersamples';
	}
	...
	*/
	...
	/**
     * Used  to calculate the number of times a record exists in the table rep_providersamples
     * It returns the number of times a record exists exists in the table rep_providersamples
     * @param array $columns
     * @param array $records
     * @param bool $printSQL
     * @return mixed
	...
	/**
     * Inserts data into the table rep_providersamples
     * if redundancy check is true, it inserts if the record if it never existed else.
     * if the record exists, it returns the number of times the record exists on the relation
     *
     * @param array $columns
     ...
     * @return mixed
     */
	public function insert_records_to_rep_providersamples(Array $columns, Array $records,$redundancy_check = false, $printSQL = false) {
	...
	/**
	 * Deletes all the records that meets the passed criteria from the table rep_providersamples
	 * @param array $columns
	 * @param array $records
	 * @param bool $printSQL
	 * @return number of deleted rows
	 */
	public function delete_record_from_rep_providersamples(Array $columns, Array $records, $printSQL = false) {
		return $this->delete_record ( $this->get_table (), $columns, $records, $printSQL );
	}
	
	/**
	 * Updates all the records that meets the passed criteria from the table rep_providersamples
	 * 
	 * @param array $columns
	 * @param array $records
	 ...
	 */
	public function update_record_in_rep_providersamples(Array $columns, Array $records, Array $where_columns, Array $where_records, $printSQL = false) {
		return $this->update_record ( $this->get_table (), $columns, $records, $where_columns, $where_records, $printSQL );
	}
	
	/**
	 * Gets an Associative array of the records in the table 'rep_providersamples' that meets the passed criteria
	 *
	 * @param $distinct
	 * @param array $columns
	 * @param array $records
	 ...
	 */
	public function fetch_assoc_in_rep_providersamples($distinct, Array $columns, Array $records, $extraSQL="", $printSQL = false) {
	 ...
	 * @return array|mixed associative
	 */
	public function query_from_rep_providersamples(Array $columns, Array $records,$extraSQL="",  $printSQL = false) {
		return $this->query ( $this->get_table (), $columns, $records,$extraSQL,$printSQL );
	}
	
	/**
	 * Gets an Associative array of the records in the table rep_providersamples that meets the passed distinct criteria
	 * 
	 * @param array $columns
	 * @param array $records
	 * @param string $extraSQL
	 ...
	 * @return array|mixed associative
	 */
	public function query_distinct_from_rep_providersamples(Array $columns, Array $records,$extraSQL="",  $printSQL = false) {
		return $this->query_distinct ( $this->get_table (), $columns, $records,$extraSQL,$printSQL );
	}
	
	/**
	 * Performs a search in the table rep_providersamples that meets the passed criteria
	 * 
	 * @param array $columns
	 * @param array $records
	 * @param string $extraSQL
	 ...
	 * @return array|mixed associative
	 */
	public function search_in_rep_providersamples(Array $columns, Array $records,$extraSQL="", $printSQL = false) {
   ...
   * @throws NullabilityException
	...
	/**
     * Gets an Associative array of the records in the table rep_providersamples that meets the distinct passed criteria
     * @param $table
     * @param array $columns
     * @param array $records
     * @param string $extraSQL
	...
	private function query_distinct($table, Array $columns, Array $records,$extraSQL="",$printSQL = false) {
	 ...
	 /**
     * Performs a search and returns an associative array of the records in the table rep_providersamples  that meets the passed criteria
     * 
     * @param $table
     * @param array $columns
     * @param array $records

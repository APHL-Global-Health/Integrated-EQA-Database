...
* ReferenceResultVl
*  
* Low level class for manipulating the data in the table reference_result_vl
*
...
class ReferenceResultVl {
	...
	*/
	...
	public function getSampleId($shipment_id,$sample_id,$sample_label,$reference_result,$control,$mandatory,$sample_score) {
		...
		$sample_id_ = $this->query_from_reference_result_vl ( $columns, $records );
	...
	public function getShipmentId($sample_id) {
		...
		$shipment_id_ = $this->query_from_reference_result_vl ( $columns, $records );
	...
	public function getSampleLabel($sample_id) {
		$columns = array ('sample_id');
		$records = array ($sample_id);
		$sample_label_ = $this->query_from_reference_result_vl ( $columns, $records );
	...
	public function getReferenceResult($sample_id) {
		$columns = array ('sample_id');
		$records = array ($sample_id);
		$reference_result_ = $this->query_from_reference_result_vl ( $columns, $records );
	...
	public function getControl($sample_id) {
		$columns = array ('sample_id');
		$records = array ($sample_id);
		$control_ = $this->query_from_reference_result_vl ( $columns, $records );
	...
	public function getMandatory($sample_id) {
		$columns = array ('sample_id');
		$records = array ($sample_id);
		$mandatory_ = $this->query_from_reference_result_vl ( $columns, $records );
	...
	public function getSampleScore($sample_id) {
		$columns = array ('sample_id');
		$records = array ($sample_id);
		$sample_score_ = $this->query_from_reference_result_vl ( $columns, $records );
	   ...
	   /**
	* Inserts data into the table[reference_result_vl] in the order below
	...
	*/
	public function insert_prepared_records($shipment_id,$sample_id,$sample_label,$reference_result,$control,$mandatory,$sample_score,$redundancy_check= false, $printSQL = false) {
		$columns = array('shipment_id','sample_id','sample_label','reference_result','control','mandatory','sample_score');
		$records = array($shipment_id,$sample_id,$sample_label,$reference_result,$control,$mandatory,$sample_score);
		return $this->insert_records_to_reference_result_vl ( $columns, $records,$redundancy_check, $printSQL );
	}

	/**
    * Delete data from the table[reference_result_vl] in the order below
    * array ('shipment_id','sample_id','sample_label','reference_result','control','mandatory','sample_score')
    * is mapped into
    * array ($shipment_id,$sample_id,$sample_label,$reference_result,$control,$mandatory,$sample_score)
    ...
    */
    public function delete_prepared_records($shipment_id,$sample_id,$sample_label,$reference_result,$control,$mandatory,$sample_score, $printSQL = false) {
    	$columns = array('shipment_id','sample_id','sample_label','reference_result','control','mandatory','sample_score');
    	$records = array($shipment_id,$sample_id,$sample_label,$reference_result,$control,$mandatory,$sample_score);
    	return $this->delete_record_from_reference_result_vl( $columns, $records, $printSQL );
    }
	...
	* @return string table name -> 'reference_result_vl' 
	*/
	public static function get_table() {
		return 'reference_result_vl';
	}
	...
	*/
	...
	/**
     * Used  to calculate the number of times a record exists in the table reference_result_vl
     * It returns the number of times a record exists exists in the table reference_result_vl
     * @param array $columns
     * @param array $records
     * @param bool $printSQL
     * @return mixed
	...
	/**
     * Inserts data into the table reference_result_vl
     * if redundancy check is true, it inserts if the record if it never existed else.
     * if the record exists, it returns the number of times the record exists on the relation
     *
     * @param array $columns
     ...
     * @return mixed
     */
	public function insert_records_to_reference_result_vl(Array $columns, Array $records,$redundancy_check = false, $printSQL = false) {
	...
	/**
	 * Deletes all the records that meets the passed criteria from the table reference_result_vl
	 * @param array $columns
	 * @param array $records
	 * @param bool $printSQL
	 * @return number of deleted rows
	 */
	public function delete_record_from_reference_result_vl(Array $columns, Array $records, $printSQL = false) {
		return $this->delete_record ( $this->get_table (), $columns, $records, $printSQL );
	}
	
	/**
	 * Updates all the records that meets the passed criteria from the table reference_result_vl
	 * 
	 * @param array $columns
	 * @param array $records
	 ...
	 */
	public function update_record_in_reference_result_vl(Array $columns, Array $records, Array $where_columns, Array $where_records, $printSQL = false) {
		return $this->update_record ( $this->get_table (), $columns, $records, $where_columns, $where_records, $printSQL );
	}
	
	/**
	 * Gets an Associative array of the records in the table 'reference_result_vl' that meets the passed criteria
	 *
	 * @param $distinct
	 * @param array $columns
	 * @param array $records
	 ...
	 */
	public function fetch_assoc_in_reference_result_vl($distinct, Array $columns, Array $records, $extraSQL="", $printSQL = false) {
	 ...
	 * @return array|mixed associative
	 */
	public function query_from_reference_result_vl(Array $columns, Array $records,$extraSQL="",  $printSQL = false) {
		return $this->query ( $this->get_table (), $columns, $records,$extraSQL,$printSQL );
	}
	
	/**
	 * Gets an Associative array of the records in the table reference_result_vl that meets the passed distinct criteria
	 * 
	 * @param array $columns
	 * @param array $records
	 * @param string $extraSQL
	 ...
	 * @return array|mixed associative
	 */
	public function query_distinct_from_reference_result_vl(Array $columns, Array $records,$extraSQL="",  $printSQL = false) {
		return $this->query_distinct ( $this->get_table (), $columns, $records,$extraSQL,$printSQL );
	}
	
	/**
	 * Performs a search in the table reference_result_vl that meets the passed criteria
	 * 
	 * @param array $columns
	 * @param array $records
	 * @param string $extraSQL
	 ...
	 * @return array|mixed associative
	 */
	public function search_in_reference_result_vl(Array $columns, Array $records,$extraSQL="", $printSQL = false) {
   ...
   * @throws NullabilityException
	...
	/**
     * Gets an Associative array of the records in the table reference_result_vl that meets the distinct passed criteria
     * @param $table
     * @param array $columns
     * @param array $records
     * @param string $extraSQL
	...
	private function query_distinct($table, Array $columns, Array $records,$extraSQL="",$printSQL = false) {
	 ...
	 /**
     * Performs a search and returns an associative array of the records in the table reference_result_vl  that meets the passed criteria
     * 
     * @param $table
     * @param array $columns
     * @param array $records
